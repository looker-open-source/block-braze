explore: canvas_core {
  ...
  sql_on: ${canvas.id} = ${subscription_event.canvas_id}
      AND ${canvas.updated_at_raw} = ${subscription_event.campaign_updated_raw}
    ;;
...
sql_on: ${canvas.id} = ${email_event.canvas_id}
      AND ${canvas.updated_at_raw} = ${email_event.canvas_updated_raw}
    ;;
relationship: one_to_many
}
join: webhook_event {
...
sql_on: ${canvas.id} = ${webhook_event.canvas_id}
      AND ${canvas.updated_at_raw} = ${webhook_event.canvas_updated_raw}
    ;;
relationship: one_to_many
}
...
sql_on: ${canvas.id} = ${in_app_message_event.canvas_id}
      AND ${canvas.updated_at_raw} = ${in_app_message_event.canvas_updated_raw}
    ;;
relationship: one_to_many
}
...
sql_on: ${canvas.id} = ${push_notification_event.canvas_id}
      AND ${canvas.updated_at_raw} = ${push_notification_event.canvas_updated_raw}
    ;;
relationship: one_to_many
}
join: canvas_variation {
  sql_on: ${canvas.id} = ${canvas_variation.canvas_id}
      AND ${canvas.updated_at_raw} = ${canvas_variation.canvas_updated_at_raw}
    ;;
  relationship: one_to_many
}
join: canvas_tag {
  sql_on: ${canvas.id} = ${canvas_tag.canvas_id}
      AND ${canvas.updated_at_raw} = ${canvas_tag.canvas_updated_at_raw}
    ;;
  relationship: one_to_many
}
join: canvas_step_seq {
  sql_on: ${canvas.id} = ${canvas_step_seq.canvas_id}
      AND ${canvas.updated_at_raw} = ${canvas_step_seq.canvas_updated_at_raw}
    ;;
  relationship: one_to_many
}
join: canvas_step {
  sql_on: ${canvas.id} = ${canvas_step.canvas_id}
      AND ${canvas.updated_at_raw} = ${canvas_step.canvas_updated_at_raw}
    ;;
  relationship: one_to_many
}
join: message_variation {
  sql_on: ${canvas_step.id} = ${message_variation.canvas_step_id}
      AND ${canvas_step.canvas_updated_at_raw} = ${message_variation.canvas_step_updated_at}
    ;;
  relationship: one_to_many
}
join: canvas_entry_event {
  sql_on: ${canvas_step.id} = ${canvas_entry_event.canvas_step_id}
      AND ${canvas.updated_at_raw} = ${canvas_entry_event.canvas_step_updated_at_raw}
    ;;
  relationship: one_to_many
}
join: entry_canvas_step {
  ...
  sql_on: ${canvas_entry_event.canvas_step_id} = ${entry_canvas_step.id}
      AND ${canvas_entry_event.canvas_updated_at_raw} = ${entry_canvas_step.canvas_updated_at_raw}
    ;;
  relationship: one_to_many
...
join: canvas_conversion_event {
  sql_on: ${canvas_step.id} = ${canvas_conversion_event.canvas_step_id}
      AND ${canvas.updated_at_raw} = ${canvas_conversion_event.canvas_updated_at_raw}
    ;;
  relationship: one_to_many
}
join: conversion_canvas_step {
  ...
  sql_on: ${canvas_conversion_event.canvas_step_id} = ${conversion_canvas_step.id}
      AND ${canvas_conversion_event.canvas_updated_at_raw} = ${conversion_canvas_step.canvas_updated_at_raw}
    ;;
  relationship: one_to_many
  fields: [canvas_step_name, id]
}
